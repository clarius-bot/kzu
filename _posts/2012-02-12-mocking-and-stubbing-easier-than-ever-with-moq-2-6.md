...
##  [Mocking and stubbing easier than ever with Moq 2.6](<http://blogs.clariusconsulting.net/kzu/mocking-and-stubbing-easier-than-ever-with-moq-2-6/> "Mocking and stubbing easier than ever with Moq 2.6")
...
Following the example above, you could create the context like so (note the added property to set the default value behavior):
    ...
    var response = context.Response; // will be non-null, an auto-generated mock!// I could set further expectations on this object, by retrieving its corresponding MockMock.Get(response).Expect(r => r.ContentType).Returns("application/xml");
...
One of the coolest things in Moq (IMO) continues to be the transparent continuum between fake/stub/mock which basically depends on how you use the one and only “moq” ![:\)](http://blogs.clariusconsulting.net/kzu/wp-includes/images/smilies/icon_smile.gif) . 
...
![](http://www.clariusconsulting.net/aggbug.aspx?PostID=102921)
